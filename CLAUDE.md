...
- All modes share `promptAndCollect` in [src/index.ts](src/index.ts), which calls `query({prompt, options: {model, resume, tools: []}})` and accumulates `assistant` text blocks until the `result` message arrives.
...
- **Token streaming display.** Flush each `assistant` content chunk to stdout immediately inside `promptAndCollect`, keep `reply` accumulating for the final-shape return. ~5-line change.

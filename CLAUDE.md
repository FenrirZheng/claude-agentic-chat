...
- **Session persistence across daemon restarts is wired, and the pointer file is per-instance.** After every successful turn the daemon writes the current `sessionId` to a file under `${XDG_STATE_HOME:-~/.local/state}/claude-chat/`. On the next `--daemon` startup that id is auto-loaded (banner: `claude-chat: resuming session <id>`) and used as the implicit `resume:` for the first turn; the resolved path is also echoed at startup (banner: `claude-chat: state file <path>`) so it's never a mystery. The **name** of that file follows `resolveStatePath` in [src/index.ts](src/index.ts), precedence highest first:
  1. `--state-file <path>` → exactly that path (relative paths resolved to absolute).
  2. `--socket` supplied on the CLI → `${stateDir}/last-session-<socket-basename>-<sha1(abspath(socket))[:8]>` — so two concurrent daemons on different sockets get independent pointers automatically, no `--fresh` dance and no cross-contamination of each other's resume id. Keyed off commander's *option source* for `socket` (was it given?), **not** a string compare against the default path — that comparison is fragile re: trailing slashes / symlinks / env-expansion timing. `path.resolve` (not `fs.realpath`) on purpose: the socket usually doesn't exist yet, so `realpath` would throw; cost is that `/tmp/foo` vs `/tmp/../tmp/foo` hash differently — pass a clean socket path if you run several daemons.
  3. neither → the literal name `last-session` — **zero migration** for every existing single-daemon user.
- `--resume <id>` overrides the loaded pointer; `--fresh` suppresses the load and starts a new conversation. To wipe state manually: `rm "$(claude-chat ... )"` — easiest is to read the `state file` banner line, or just `rm -i ~/.local/state/claude-chat/last-session*`. One-shot and REPL modes intentionally don't read or write any of this — see `stateDir` / `resolveStatePath` / `loadLastSession` / `saveLastSession` in [src/index.ts](src/index.ts).
- **The socket itself is still a per-user singleton at the default path.** Two Claude Code sessions on one machine that both want a persistent side-claude daemon need distinct `--socket` paths regardless — with the per-instance pointer above, that one flag now isolates everything. (For a throwaway side-task that doesn't need persistence, one-shot `claude-chat --json "Q"` touches neither the socket nor any state file — no coupling at all.)
...
- **`cleanup-state` command.** Per-instance pointer files (`last-session-<basename>-<hash>`) accumulate under `~/.local/state/claude-chat/` as users experiment with `--socket` paths; nothing prunes them. A `claude-chat cleanup-state [--keep-default]` (delete every `last-session-*` whose socket no longer has a live daemon) would tidy it. Manual workaround for now: `rm -i ~/.local/state/claude-chat/last-session-*`. (Background in [docs/daemon-state-isolation-plan.md](docs/daemon-state-isolation-plan.md), critique consult point (e).)
